...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('config-setshift')
        .setDescription('Set the channel shift messages go to.')
        .addChannelOption(option =>
            option
                .setName('channel')
                .setDescription('The channel used to post shift messages.')
                .setRequired(true)),
    async execute(interaction) {
        try {
            ...
            if (serverExists) {
                await Server.findOneAndUpdate({ serverId }, { $set: { shiftID: ID } });
            } else {
                const newServer = new Server({ serverId, shiftID: ID });
                await newServer.save();
            }

            await interaction.reply(`Sessions channel updated successfully to ${trainingStartMessage}.`);
        } catch (error) {
            console.error('Error setting shift channel', error);
            await interaction.reply('Error setting shift channel. Please try again later.');
        }
    },
};
